  ...
  return (
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
      <p className="text-muted mb-4 sm:mb-0">
        Showing <span className="font-medium text-primary">{totalItems}</span>{" "}
        books
      </p>
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex border border-gray-300 rounded-lg overflow-hidden">
          ...
          >
            <Grid
              className={`h-5 w-5 ${viewMode === "grid" ? "text-accent" : "text-muted"}`}
            />
          </button>
          <button
          ...
          >
            <List
              className={`h-5 w-5 ${viewMode === "list" ? "text-accent" : "text-muted"}`}
            />
          </button>
        </div>
      </div>
